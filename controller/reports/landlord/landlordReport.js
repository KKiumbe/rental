...
async function getLandlordRentReport(req, res) {
  ...
  const month = req.query.month || new Date().toISOString().slice(0, 7); // Default to current month (2025-06)
  ...
  try {
    ...
    // Fetch landlords with buildings, units, customers, and invoices
    ...
    // Fetch all payments for the tenant in the specified month
    ...
    console.log(`Total payments for tenantId ${tenantId} in ${month}: ${allPayments.length}`);
    landlords.forEach((landlord) => {
      const linkedPaymentCount = landlord.buildings
        ...
        .length;
      console.log(`Landlord ${landlord.firstName} ${landlord.lastName} has ${linkedPaymentCount} linked payments`);
    });
    ...
    doc.font('Helvetica').fontSize(12).text(`Landlord Rent Report - ${month}`, { align: 'center' });
    doc.moveDown(1);

    // Table setup
    const columnWidths = [60, 90, 40, 100, 100]; // For Landlord Name, Property, Total Income, Management Fees, Net Amount
    const startX = 50;
      ...
      if (!hasBuildingsWithIncome) {
        ...
        for (const building of landlordEntry.buildings.values()) {
          ...
          drawTableRow(rowY, [
            ...
            building.name,
            building.totalIncome > 0 ? `$${building.totalIncome.toFixed(2)}` : 'No rent collected',
            building.totalIncome > 0 ? `$${building.managementFees.toFixed(2)}` : '$0.00',
            building.totalIncome > 0 ? `$${netAmount.toFixed(2)}` : '$0.00',
          ]);
          rowY += 30;
        ...
        drawTableRow(
          ...
          [
            ...
            landlordEntry.totalIncome > 0 ? `$${landlordEntry.totalIncome.toFixed(2)}` : 'No rent collected',
            landlordEntry.totalIncome > 0 ? `$${landlordEntry.totalManagementFees.toFixed(2)}` : '$0.00',
            landlordEntry.totalIncome > 0 ? `$${landlordNetAmount.toFixed(2)}` : '$0.00',
          ],
          false,
          true // Bold
        );
    ...
    drawTableRow(
      ...
      [
        '',
        'Grand Total',
        grandTotalIncome > 0 ? `$${grandTotalIncome.toFixed(2)}` : 'No rent collected',
        grandTotalIncome > 0 ? `$${grandTotalManagementFees.toFixed(2)}` : '$0.00',
        grandTotalIncome > 0 ? `$${grandTotalNet.toFixed(2)}` : '$0.00',
      ],
      false,
      true // Bold
    );
...
async function getIncomePerBuilding(req, res) {
  ...
  try {
    ...
    // Fetch all payments with Customer, Unit, and Building
    const payments = await prisma.payment.findMany({
      where: {
        tenantId,
        //createdAt: { gte: startDate, lte: endDate },
        customer: { status: 'ACTIVE' }, // Single relation filter
      },
      select: {
        id: true,
        amount: true,
    ...
    payments.forEach((p) => {
      console.log(
        `Payment ${p.id}: Amount=${p.amount}, CustomerId=${p.customer?.id || 'null'}, UnitId=${p.customer?.unit?.id || 'null'}, BuildingId=${p.customer?.unit?.building?.id || 'null'}`
      );
    });
    ...
    generatePDFHeader(doc, tenant);
    doc.font('Helvetica').fontSize(12).text(`Income per Building Report - ${month}`, { align: 'center' });
    doc.moveDown(1);
    ...
    if (landlordBuildings.size === 0) {
      doc.font('Helvetica').fontSize(10).text('No income recorded for this period.', startX, rowY, { align: 'center' });
      doc.end();
      return;
    }
    ...
    for (const [landlordName, buildings] of landlordBuildings) {
      ...
      for (const buildingEntry of buildings.values()) {
        ...
        drawTableRow(doc, rowY, [
          '',
          buildingEntry.name,
          buildingEntry.totalIncome > 0 ? `$${buildingEntry.totalIncome.toFixed(2)}` : 'No rent collected',
          buildingEntry.totalIncome > 0 ? `$${buildingEntry.managementFees.toFixed(2)}` : '$0.00',
          buildingEntry.totalIncome > 0 ? `$${netAmount.toFixed(2)}` : '$0.00',
        ], columnWidths, startX);
        rowY += 30;
...
async function getIncomePerLandlord(req, res) {
  ...
  try {
    ...
    // Fetch payments with valid customerId, Customer, Unit, and Building
    const payments = await prisma.payment.findMany({
      where: {
        tenantId,
        //createdAt: { gte: startDate, lte: endDate },
    ...
    generatePDFHeader(doc, tenant);
    doc.font('Helvetica').fontSize(12).text(`Income per Landlord Report - ${month}`, { align: 'center' });
    doc.moveDown(1);
